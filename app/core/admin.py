...
class UserAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Account Info', {
            'fields': ('is_active', 'is_staff', 'created_at')
        })
    )
class AdvertiserAdmin(admin.ModelAdmin):
    list_display = ['advertiser_id', 'name', 'industry', 'country', 'is_active', 'created_at']
    list_filter = ['industry', 'country', 'is_active']
    search_fields = ['name', 'industry']


class AdAdmin(admin.ModelAdmin):
    list_display = ['ad_id', 'advertiser', 'ad_title', 'ad_format', 'is_active', 'created_at']
...
class SpendSummaryAdmin(admin.ModelAdmin):
    list_display = ['advertiser', 'window_start', 'window_end', 'gross_spend', 'net_spend', 'can_serve']
    list_filter = ['can_serve', 'advertiser']


class BudgetEventAdmin(admin.ModelAdmin):
    list_display = ['event_id', 'advertiser', 'new_budget_value', 'event_time', 'source']
