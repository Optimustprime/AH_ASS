...
class BudgetManager:
    ...
    def get_latest_budget(advertiser: Advertiser) -> Optional[BudgetEvent]:
        ...
        return (
            BudgetEvent.objects.filter(advertiser=advertiser)
            .order_by("-event_time")
            .first()
        )
    ...
    def calculate_daily_spend(
        advertiser: Advertiser, window_start: datetime, window_end: datetime
    ) -> float:
        ...
        return click_events.aggregate(total=Sum("amount"))["total"] or 0.0
...
class SpendSummaryManager:
    ...
    def get_or_create_spend_summary(
        self, advertiser: Advertiser, window_start: datetime, window_end: datetime
    ) -> Optional[SpendSummary]:
        """Get existing spend summary or create new one."""
        return SpendSummary.objects.filter(
            advertiser=advertiser, window_start=window_start, window_end=window_end
        ).first()

    def update_spend_summary(
        self, advertiser_id: int, click_event_date: datetime
    ) -> Optional[SpendSummary]:
        ...
        try:
            ...
            window_start, window_end = self.budget_manager.get_daily_window(
                click_event_date
            )
            ...
            net_spend = self.budget_manager.calculate_net_spend(
                gross_spend, budget_value
            )
            ...
            logger.error(
                f"Error updating spend summary for advertiser {advertiser_id}: {e}"
            )
            return None
...
class BudgetUpdateHandler:
    ...
    def update_advertiser_budget(
        self, advertiser_id: int, new_budget_value: float, source: str = "API Update"
    ) -> dict:
        ...
        try:
            ...
            if new_budget_value < 0:
                return {"status": "error", "message": "Budget value cannot be negative"}
            ...
            if spend_summary:
                logger.info(
                    f"Budget updated for advertiser {advertiser_id}: {new_budget_value}"
                )
                return {
                    "status": "success",
                ...
                logger.warning(
                    f"Budget updated but spend summary update failed for advertiser {advertiser_id}"
                )
                return {
            ...
            logger.error(f"Advertiser {advertiser_id} not found")
            return {"status": "error", "message": "Advertiser not found"}
        except Exception as e:
...
def create_or_update_spend_summary(
    advertiser_id: int, click_event_date: datetime
) -> Optional[SpendSummary]:
...
def update_budget(request):
    ...
    try:
        ...
        if not advertiser_id:
            return JsonResponse(
                {"status": "error", "message": "advertiser_id is required"}, status=400
            )

        if not new_budget_value:
            return JsonResponse(
                {"status": "error", "message": "new_budget_value is required"},
                status=400,
            )
